...
def create_model(aux, num_classes, pretrain=True):
    ...
    if pretrain:
        ...
        if num_classes != 21:
            # The provided pre-trained weights are for 21 classes (including background).
            # If training a different dataset, remove the weights related to the class to avoid shape inconsistency error.
...
def main(args):
    ...
    # Create learning rate update strategy, here update every step.次
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 50 == 0:
            torch.save(save_file, "save_weights/deeplabv3_model_{}.pth".format(epoch))
...
def parse_args():
    ...
    parser.add_argument("--epochs", default=301, type=int, metavar="N",
